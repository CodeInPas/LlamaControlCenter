...
begin
  ...
  begin
    ATotal := MemStatus.ullTotalPhys;
    AAvailable := MemStatus.ullAvailPhys;
    Result := True;
  end
  else
  begin
...
begin
  ...
  begin
    ATotal := TotalMem;
    AAvailable := TotalMem div 2; // Approximate available memory
    Result := True;
  end
  else
  begin
...
begin
  ...
  // Heuristic estimation for physical core mapping
  if ALogical >= 4 then
  ...
  {$IFDEF WINDOWS}
  AName := GetEnvironmentVariable('PROCESSOR_IDENTIFIER');
  if AName = '' then
        ...
        try
          ...
          begin
            ...
            begin
              ...
              GPU.Name := WideCharToString(AdapterDesc.Description);
              GPU.TotalVRAMBytes := Int64(AdapterDesc.DedicatedVideoMemory);
              GPU.FreeVRAMBytes := GPU.TotalVRAMBytes; // Baseline initialization
              GPU.UsedVRAMBytes := 0;
...
begin
  ...
  // Select GPU with highest dedicated VRAM as primary
...
begin
  ...
  if TotalLayersSafe <= 0 then TotalLayersSafe := 33; // Default 7B/8B layer heuristic
  ...
  if AKVCacheType = 'q8_0' then
    ...
    BytesPerKVElem := 2.0; // f16

  // Heuristic KV cache footprint: 2 (K+V) * layers * hidden_dim * tokens
  KVCacheBytes := Round(2 * TotalLayersSafe * 4096 * BytesPerKVElem * (AContextTokens / 2048.0));
  TotalRequiredBytes := AModelFileSizeBytes + KVCacheBytes + (384 * 1024 * 1024); // 384MB CUDA/Context buffer
  ...
  begin
    // Leave 768 MB VRAM headroom for Windows OS/Display Compositor
    ...
    begin
      ...
      Result.RecommendedGPULayers := TotalLayersSafe + 1; // 99+ layers
