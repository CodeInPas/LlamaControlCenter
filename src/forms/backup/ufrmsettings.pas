...
begin
  ...
  try
    ...
    begin
      SecObj := TJSONObject(SectionData);
      FCurrentConfig.LlamaBinDir := GetJSONString(SecObj, 'llama_bin_dir', 'bin/llama-bin');
      FCurrentConfig.ServerBinary := GetJSONString(SecObj, 'server_binary', 'bin/llama-bin/llama-server.exe');
      FCurrentConfig.CliBinary := GetJSONString(SecObj, 'cli_binary', 'bin/llama-bin/llama-cli.exe');
      FCurrentConfig.QuantizeBinary := GetJSONString(SecObj, 'quantize_binary', 'bin/llama-bin/llama-quantize.exe');
      FCurrentConfig.BenchBinary := GetJSONString(SecObj, 'bench_binary', 'bin/llama-bin/llama-bench.exe');
      FCurrentConfig.SplitBinary := GetJSONString(SecObj, 'split_binary', 'bin/llama-bin/llama-gguf-split.exe');
    ...
    begin
      ...
      FCurrentConfig.LastUsedProfile := GetJSONString(SecObj, 'last_used_profile', 'Default-Balanced');
    end;
